...
string Diff(string term, char var)
{
    ...
    else
    {
        ...
        { // CASE: a*sin(u) or a*sin^1(u)
            ...
            if (trigon[0] == "sin" || trigon[0] == "csc()*cot()" || trigon[0] == "csc^2")
                a = -1;
        ...
        { // CASE: a*sin^n(u)
            ...
            for (unsigned short i = 0; i < u[0].size() && u[0][i] != '('; i++)
            { // find fisrt '(' pos
                fisrtParPos++;
            }
