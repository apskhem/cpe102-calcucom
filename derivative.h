...
string Diff(string term, char var) {
    ...
    array<unsigned short> trigonIndex, varIndex;
    unsigned short outerPair = 0; // for (...)(...)
    ...
    if (u.length == 0) {
        switch (term[varIndex[0] + 1]) {
            case '^': { // CASE: ax^n
                ...
                string strN = "";
                for (unsigned short i = tpos; i < term.length && isNum(term[i]); i++) {
                    strN += term[i];
                }
        ...
        if (trigon.length > 0) { // CASE: a*sin(u) or a*sin^1(u)
            ...
            for (unsigned short i = 0; i < chainDiff.length; i++) {
